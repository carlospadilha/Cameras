...
function inicCanvas(){
	...
	$("#cam3Rec").imgSnap(3, cam.strm3, false, true)
//	$("#canvas").cloth(true)
//	$("#vidTst").webRTC(true)
}
...
(function($){
	...
	$.fn.camSave = function(){	// $(canvas).camSave(name, mode, source, outWidth, outHeight, frameRate, show, flip)
		...
		if (!elem) { cp.msgErr('$("' + this.selector + '").imgMx não encontrado !'); return this }
		if (!arguments.length || arguments.length < 1) return this;		// $(...).camSave()		or		$(...).camSave(file, ...)
		$('#canvas').hide();
		for (var i = 0, args = []; i < arguments.length; i++) args[i] = arguments[i];

		switch (args[1]){
			case "rec":	 setup();	imgRec(); break;
			case "stop": imgStop();	break;
			case "play": imgPlay();	break;
			default:	 imgPlay();	break;
		}
		return this;

		function setup(){
			var cam = args[0].substr(3);
			var canvas = elem.children[0]/* <canvas...> */;
			if (!canvas){
				canvas = document.createElement('canvas');	// <canvas...> não existe, crie-o
				canvas.id = "rec_" + args[0];
				canvas.e = {
					file: args[0], mode: args[1], src: args[2], outW: args[3], outH: args[4], fRate: 1000 / args[5], show: args[6], flip: args[7],
					frames: [], stop: false, tmr: 0, size: 0, count: 0, videoEncoder: new Whammy.Video(), time: 0, filename: '', ctx: canvas.getContext('2d'),
					frm: { frameNumber: 0, EBML: {}, duration: 0, segment: {}, cues: {}, clusterTimecode: 0, clusterDuration: 0, clusterCounter: 0, positionArray: [] }
				};
				$(elem).append(canvas);
			}
			canvas.e.stop = false;
		...
		function imgRec(){
			var canvasChild = elem.children[0]/* <canvas...> */, e = canvasChild.e;
			e.show ? $(canvasChild).show() : $(canvasChild).hide();
			$("#vid").hide()	// esconda video e dowload-link
			e.filename = e.file + '.webm';
			imgRecLoop()
			...
			function imgRecLoop(){
				...
				imgLoad(e.src + (e.file == "cam2" ? "" : ("?" + cp.secTime())), e.flip, function(x){
					camRec($("#" + args[0] + "Rec")[0], args[0].substr(3), x, !e.stop)
					if (!e.stop) e.tmr = setTimeout(imgRecLoop, 10)	/*raf = requestAnimationFrame(imgRec)*/
					else { imgStop(); return };
					if (!x.img) return;

					var kW = x.w / 1280;
					x.ctx.font = 28 * kW + "px Arial"; x.ctx.fillStyle = "yellow"; ctxText(x.ctx, now(e.file), 3, 25 * x.h / 720, 4 * kW);
					var image = x.cvs.toDataURL('image/webp', 1)/* image/jpeg (faster) */, frameRate = cp.timeDate() - time;
					e.videoEncoder.image(image, frameRate > 1000 ? 1000 : frameRate, e.frm, true);	/*e.videoEncoder.add(image, frameRate > 1000 ? 1000 : frameRate);*/

					if (e.show) e.ctx.drawImage(x.cvs, 0, 0, e.outW, e.outH)	// mostra com outW e outH

					time = cp.timeDate();
					e.stop = (e.size += image.length) > (600e6 / .75);
					e.time += frameRate;
					showMsg(canvasChild, " # " + (e.count++) + " (" + (e.size * .00000075).toFixed(1) + " MB) ")
				})
			}
		}
		...
		function imgStop(){
			...
			if (!canvas) { cp.msgErr("canvas não criado"); return }
			var e = canvas.e;
			e.stop = true; clearTimeout(e.tmr);	/*cancelAnimationFrame(raf);*/
			if (!e.frm.frameNumber) return	// blob já gerado ou nem captou frames
			else var blob = e.videoEncoder.gera_EBML(e.frm, true)
			e.frm = []
...
/*			data = new Uint8Array(5e8);
			...
			$("#cvsMsg1").texto("")
			blobSave(blob, e.filename, function(x){
				showMsg(canvas, " size: " + (blob.size / 1e6).toFixed(1) + " MB ")
			})
		...
		function imgPlay(){
			var video = $("#vid")[0].children[0], dnld = $("#vid")[0].children[1];
			...
			dnld.href = video.src = 'php/streamRead.php?file=' + "../buffer/" + args[0] + ".webm";
		}

		function showMsg(x, txt){
			var min = x.e.time / 60000;
		...
		function chunkBlob(blob, file){
			var chunkSize = /*1048576 * 1024 / 10*/blob.size, size = blob.size, start = 0;
			$("#cvsMsg1").texto("")
			dir.delFile("../buffer/" + file, function(x){ if (x == "deleted" || x == "nex") chunkLoop() })
			function chunkLoop(){
				if ( start < size ){
					var dif = start + chunkSize, endReal = dif > size ? size : dif, end = dif;
					chunkSave(blob.slice(start, end), file, function(x){
						start = end;
		...
		function chunkSave(blob, fileName, callback){	// fileName: .webm (fileType = video) ou .wav .ogg (fileType = audio)
			var fileType = 'video'/* ou "audio" */, formData = new FormData();
			formData.append(fileType + '-filename', fileName)
			formData.append(fileType + '-blob', blob);
			sendForm('php/chunkBlob.php'/*'php/saveBlob.php'*/, formData, callback);
			function sendForm(url, data, callback){
				var xhr = new XMLHttpRequest();
				xhr.onreadystatechange = function(){
					if (xhr.readyState == 4 && xhr.status == 200){
						if (/Warning/.test(xhr.responseText)) alert(xhr.responseText);
					}
				};
				xhr.upload.addEventListener("progress", uploadProgress, false);
				function uploadProgress(e){ /*if (e.lengthComputable) $("#cvsMsg1").texto(Math.round(e.loaded * 100 / e.total).toString() + '%')*/ }
		...
		function blobSave(blob, fileName, callback){	// fileName: .webm (fileType = video) ou .wav .ogg (fileType = audio)
			var fileType = 'video'/* ou "audio" */, formData = new FormData();
			formData.append(fileType + '-filename', fileName);
			formData.append(fileType + '-blob', blob);
			sendForm('php/saveBlob.php', formData, callback);
			function sendForm(url, data, callback){
				var xhr = new XMLHttpRequest();
				xhr.onreadystatechange = function(){
					if (xhr.readyState == 4 && xhr.status == 200){
